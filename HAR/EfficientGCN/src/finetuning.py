    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            for num, (x, y, _) in enumerate(eval_iter):
                ...
                num_top3 += sum([y[n] in reco_top3[n,:] for n in range(x.size(0))])
        ...
        acc_top3 = num_top3 / num_sample
        eval_loss = sum(eval_loss) / len(eval_loss)
        eval_time = time() - start_eval_time
        eval_speed = len(self.eval_loader) * self.eval_batch_size / eval_time / len(self.args.gpus)
        logging.info('Top-1 accuracy: {:d}/{:d}({:.2%}), Top-3 accuracy: {:d}/{:d}({:.2%}), Mean loss:{:.4f}'.format(
            num_top1, num_sample, acc_top1, num_top3, num_sample, acc_top3, eval_loss
        ))
