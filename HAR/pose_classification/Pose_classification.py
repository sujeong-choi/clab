...
class PoseClassifier(object):
    ...
    for file_name in file_names:
      ...
      with open(os.path.join(pose_samples_folder, file_name)) as csv_file:
        ...
        for row in csv_reader:
          assert len(row) == n_landmarks * n_dimensions + 2, 'Wrong number of values: {} , n_landmarks : {}, n_dimentions : {}'.format(len(row),n_landmarks,n_dimensions)
          # print(row[2:])
  ...
  def __call__(self, pose_landmarks):
    ...
    # print('pose_landmarks = ',pose_landmarks[0])
    # Get given pose embedding.
    pose_embedding = self._pose_embedder(pose_landmarks)
    # print('pose_embedding = ',pose_embedding[0])
    flipped_pose_embedding = self._pose_embedder(pose_landmarks * np.array([-1, 1, 1]))
    # print('flipped_pose_embedding = ',flipped_pose_embedding[0])
    ...
    # print('max_dist_heap = ',max_dist_heap[0])
    ...
    mean_dist_heap = []
    for distance, sample_idx in max_dist_heap:
      print()
    ...
    PAINTING_THRESHOLD = 6
    class_names = ['painting' if dis<PAINTING_THRESHOLD else 'nothing' for dis, sample_idx in mean_dist_heap]
...
def main(video_path, out_video_path):
# Specify your video name and target pose class to count the repetitions.
  video_path = video_path # 'data/long/input (1).mp4'
  class_name='painting'
  out_video_path = out_video_path# 'data/output/6_5/input (1).mp4'
  ...
  with tqdm.tqdm(total=video_n_frames, position=0, leave=True) as pbar:
    while True:
      ...
      if pose_landmarks is not None:
        ...
        thickness = 2
        # Define the text string and position
        position = (int(10), int(20))

        # Classify the pose on the current frame.
        pose_classification = pose_classifier(pose_landmarks)

        if 'painting' not in pose_classification:
          text = 'nothing'
        else:
          if pose_classification['painting'] >= K_THRESHOLD :  text = 'painting'

        cv2.putText(output_frame, text, position, font, scale, color, thickness)
...
if __name__ == "__main__":
    iroot = 'data/short'
    oroot = 'data/output/6_5'
    for input_name in os.listdir(iroot):
        main(os.path.join(iroot,input_name),os.path.join(oroot,input_name))
